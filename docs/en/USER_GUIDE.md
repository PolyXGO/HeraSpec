...
3. **Add project-specific skills:**
   ...
   # Add WordPress Plugin Check skill
   heraspec skill add plugin-check --project-type wordpress
   # Note: Requires "Plugin Check (PCP)" plugin installed and DB_HOST='127.0.0.1' in wp-config.php
   ```
...
HeraSpec provides specialized skills for WordPress plugin development:

**1. Plugin Check skill:**
```bash
heraspec skill add plugin-check --project-type wordpress-plugin
```
- Runs WordPress Plugin Check (PCP) tool
- Fixes coding standard issues automatically
- Requires Plugin Check plugin installed in WordPress

**2. Plugin Directory skill (NEW):**
```bash
heraspec skill add plugin-directory --project-type wordpress-plugin
```
- Reviews plugin against **18 WordPress.org Plugin Directory Guidelines**
- Generates detailed compliance report
- Requires user confirmation before making changes
- Essential for WordPress.org plugin submission

**Example usage in tasks.md:**
```markdown
## 1. Review Plugin Compliance (projectType: wordpress-plugin, skill: plugin-directory)
- [ ] 1.1 Scan plugin source code
- [ ] 1.2 Generate compliance report for 18 guidelines
- [ ] 1.3 Wait for user approval before fixing
```

**Example prompt for AI agent:**
> "Use skill plugin-directory to review plugin `my-plugin` for WordPress.org compliance. Generate a compliance report with all 18 guidelines and wait for my approval before fixing."

**The 18 WordPress Plugin Directory Guidelines:**
1. GPL Compatibility
...
```bash
# Update skill to latest version from HeraSpec core
